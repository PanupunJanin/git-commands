...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > This error occurs because the local branch dev-foo doesn't have an associated upstream branch on the remote repository. 
   > Git doesn't know where to push the branch. You need to set an upstream branch to fix this.
   
   > "upstream branch" is a link between your local dev-foo and a branch on the remote repository. 
